...
const PageItem = ({ content, isCurrent }: { content: number | string; isCurrent: boolean }) => (
    <li className="h-10 w-10">
        {content === '...' ? (
            <span className="flex-center h-full w-full text-blue-100">...</span>
        ) : (
            <Link
                href={{ query: { page: content } }}
                className={classNames(
                    'flex-center h-full w-full rounded-full border hover:border-blue-active hover:text-blue-active',
    ...
    return (
        <div className="mt-8 flex justify-center gap-x-1 font-medium">
            <Link
                ...
                className={classNames(
                    'flex-center h-10 rounded-full border border-blue-20 px-6 text-blue-100 hover:border-blue-active hover:text-blue-active',
                    { 'pointer-events-none opacity-30': currentPage === 1 },
                )}
            >
                {t('paginator.prev')}
            </Link>
            <ol className="sm:flex-center hidden gap-x-1">{renderPageNumbers()}</ol>
            <Link
                href={{ query: { page: currentPage + 1 } }}
                className={classNames(
                    'flex-center h-10 rounded-full border border-blue-20 px-6 text-blue-100 hover:border-blue-active hover:text-blue-active',
