...
export const FormButtonMinor = ({
    ...
    isDisabled,
    onClick,
}: FormButtonMinorProps) => {
    return (
        <button
            className={classNames(
                'hover-animated flex-center relative h-8 cursor-pointer gap-x-2 px-6 text-small-bold enabled:hover:text-white disabled:cursor-no-drop disabled:opacity-30',
                {
                    'bg-blue-20 text-blue-100 enabled:hover:bg-blue-active': variant === 'blue',
                    'bg-orange-20 text-orange-100 enabled:hover:bg-orange-active': variant === 'orange',
                    'bg-red-20 text-red-100 enabled:hover:bg-red-active': variant === 'red',
                },
                className,
            )}
        ...
        >
            ...
            <span className={classNames('flex-center gap-x-2', { 'opacity-0': isLoading })}>{children}</span>
        </button>
    )
}
