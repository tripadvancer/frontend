import classNames from 'classnames'

type ActionControlProps = {
    isActivated: boolean
}

export const ActionControl = ({ isActivated }: ActionControlProps) => {
    return (
        <div
            className={classNames(
                'hover-animated flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg bg-custom-blue-20 text-custom-blue-100 hover:bg-custom-blue-active hover:text-white',
                {
                    'bg-custom-blue-active text-white': isActivated,
                },
            )}
        >
            <svg width="16" height="4" viewBox="0 0 16 4" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path d="M10 2C10 3.10457 9.10457 4 8 4C6.89543 4 6 3.10457 6 2C6 0.89543 6.89543 0 8 0C9.10457 0 10 0.89543 10 2Z" />
                <path d="M4 2C4 3.10457 3.10457 4 2 4C0.89543 4 0 3.10457 0 2C0 0.89543 0.89543 0 2 0C3.10457 0 4 0.89543 4 2Z" />
                <path d="M16 2C16 3.10457 15.1046 4 14 4C12.8954 4 12 3.10457 12 2C12 0.89543 12.8954 0 14 0C15.1046 0 16 0.89543 16 2Z" />
            </svg>
        </div>
    )
}
